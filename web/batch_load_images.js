...
>>>>>>> b6e6985 (Update external upload workflow documentation)
    ...
    queueOneBtn.onclick = async () => {
        ...
        if (wIndex) { wIndex.value = 0; wIndex.callback?.(wIndex.value); }
        await queueCurrent(node);
    };
<<<<<<< HEAD
...
function createNodeSelectSwitchUI(node) {
    ...
    selectBtn.prepend(selectedText);

    const menu = document.createElement("div");
    menu.style.cssText = `display:none;position:absolute;z-index:20;left:0;right:0;top:43px;background:var(--comfy-input-bg);border:1px solid var(--border-color);border-radius:4px;overflow:hidden;box-shadow:0 8px 18px rgba(0,0,0,0.35);`;
    ...
    const redraw = () => {
        ...
        btn.addEventListener("click", async (e) => {
            e.preventDefault();
            e.stopPropagation();
            const action = labels.includes(buttonWidget?.value) ? buttonWidget.value : labels[0];
