...
// 💡 回归最官方、最安全的隐藏方式，绝不使用负数高度，杜绝节点崩溃拉伸
function neutralizeWidget(w) {
    if (!w) return;
    w.type = "hidden";
    w.hidden = true;
    w.computeSize = () => [0, 0]; // 必须为 0，不能为负数
    w.draw = () => {}; 
...
function createBrowserUI(node) {
    const container = document.createElement("div");
    container.style.cssText = "width:100%;box-sizing:border-box;padding:8px;background:var(--comfy-menu-bg);border:1px solid var(--border-color);border-radius:6px;margin:5px 0 0 0;pointer-events:auto;display:flex;flex-direction:column;";

    const btnRow = document.createElement("div"); btnRow.style.cssText = "display:flex;gap:6px;margin-bottom:8px;flex-shrink:0;";
    const mkBtn = (label) => {
        const b = document.createElement("button"); b.textContent = label;
        b.style.cssText = "flex:1;padding:4px 8px;background:var(--comfy-input-bg);color:var(--input-text);border:1px solid var(--border-color);border-radius:4px;cursor:pointer;font-size:12px;";
        return b;
    };
    ...
    const info = document.createElement("div"); info.style.cssText = "font-size:12px;color:#4a6;margin-bottom:6px;font-weight:bold;flex-shrink:0;";
    const grid = document.createElement("div"); grid.style.cssText = "display:grid;grid-template-columns:repeat(auto-fill,minmax(96px,1fr));gap:6px;flex:1;min-height:60px;overflow-y:auto;background:var(--comfy-input-bg);padding:6px;border-radius:4px;";
