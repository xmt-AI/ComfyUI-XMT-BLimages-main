...
class BatchLoadImages:
    ...
    def load_images(self, image_list: str, max_images: int, mode: str, index: int, append=None):
    ...
    @classmethod
    def IS_CHANGED(s, image_list: str, max_images: int, mode: str, index: int, append=None):
        m = hashlib.sha256()
        names = [x.strip() for x in (image_list or "").splitlines()]
        names = [x for x in names if x]
        append_name = str(append or "").strip()
    ...
    @classmethod
    def VALIDATE_INPUTS(s, image_list: str, max_images: int, mode: str, index: int, append=None):
        names = [x.strip() for x in (image_list or "").splitlines()]
        names = [x for x in names if x]
        append_name = str(append or "").strip()
        if append_name and append_name not in names:
