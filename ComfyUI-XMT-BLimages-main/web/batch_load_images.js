...
async function queueFirstImage(node) {
    const wMode = getWidgetByName(node, "mode");
    const wIndex = getWidgetByName(node, "index");
    if (wMode) { wMode.value = "single"; wMode.callback?.(wMode.value); }
    if (wIndex) {
        wIndex.value = 0;
...
function createNodeSelectSwitchUI(node) {
    ...
    const container = document.createElement("div");
    container.style.cssText = `width:100%;box-sizing:border-box;padding:4px 0 0;margin:0;color:${XMT_TEXT_COLOR};pointer-events:auto;background:transparent;border:none;`;

    const select = document.createElement("select");
    select.style.cssText = `width:100%;box-sizing:border-box;margin-bottom:8px;padding:8px;background:var(--comfy-input-bg);color:${XMT_TEXT_COLOR};border:1px solid var(--border-color);border-radius:4px;font-size:13px;`;
    ...
    const mkBtn = (label) => {
        ...
        b.textContent = label;
        b.style.cssText = `flex:1;padding:8px;background:var(--comfy-input-bg);color:${XMT_TEXT_COLOR};border:1px solid var(--border-color);border-radius:4px;cursor:pointer;font-size:13px;`;
        return b;
    };

    const redraw = () => {
        let current = legacyLabelMap[buttonWidget?.value] || buttonWidget?.value || labels[0];
        if (!labels.includes(current)) {
            current = labels[0];
            if (buttonWidget) {
                buttonWidget.value = current;
        ...
        btn.addEventListener("click", async (e) => {
            e.preventDefault();
            e.stopPropagation();
            const action = labels.includes(select.value) ? select.value : labels[0];
            ...
            for (const target of targets) {
                if (action === "重新上传") openMultiSelect(target, { replace: true, infoElem: info });
                else if (action === "追加图片") openMultiSelect(target, { replace: false, infoElem: info });
                else if (action === "上传文件夹") openFolderSelect(target, { replace: true, infoElem: info });
                else if (action === "逐张执行") await queueAllSequential(target);
                else if (action === "执行首张") {
    ...
    select.onchange = (e) => {
        e.stopPropagation();
        if (buttonWidget) {
            buttonWidget.value = select.value;
            buttonWidget.callback?.(buttonWidget.value);
        }
        redraw();
    };

    container.append(select, buttonWrap, info);
            ...
            this.onResize = function () {
                ...
                if (!this._batchLoadImagesApplyingLayout) {
                    ...
                    this._batchLoadImagesResizeTimer = setTimeout(() => {
                        this._batchLoadImagesUI?.relayout?.({ fitNode: true });
                    }, 180);
                }
                return result;
            };
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        nodeType.prototype.onNodeCreated = function () {
            const r = origOnNodeCreated?.apply(this, arguments);
            applyNodeTextColor(this);
            this.title = "熊猫头封装节点切换";
            const ui = createNodeSelectSwitchUI(this);
            this._nodeSelectSwitchUI = ui;
            this.addDOMWidget("node_select_switch", "customwidget", ui.container);
