...
def _parse_control(control):
    ...
    if isinstance(control, dict):
        return control
    text = str(control).strip()
    if not text:
        return {}
    try:
        data = json.loads(text)
        return data if isinstance(data, dict) else {"action": str(data)}
    except (TypeError, ValueError, json.JSONDecodeError):
        return {"action": text}


def _control_action(control_data, control=None):
    action = ""
    if isinstance(control_data, dict):
        action = control_data.get("action") or control_data.get("button") or ""
    if not action and control:
        action = str(control)
    action = str(action)
    if "\\u" in action:
        try:
            action = action.encode("utf-8").decode("unicode_escape")
        except UnicodeDecodeError:
            pass
    return action


def _is_single_action(action):
    return (
        action in ("\u9010\u5f20\u6267\u884c", "\u6267\u884c\u9996\u5f20")
        or "\u9010\u5f20" in action
        or "\u9996\u5f20" in action
...
def _is_first_action(action):
    return action == "\u6267\u884c\u9996\u5f20" or "\u9996\u5f20" in action
...
class BatchLoadImages:
    ...
    def load_images(self, image_list: str, max_images: int, mode: str, index: int = None, append=None, control=None):
        control_data = _parse_control(control)
        control_action = _control_action(control_data, control)
        if _is_single_action(control_action):
            mode = "single"
            index = 0 if _is_first_action(control_action) else control_data.get("index", index)
        elif not control:
            mode = "single"
            index = 0
        ...
        for name in names:
            ...
            for frame in ImageSequence.Iterator(img):
                ...
                if mode == "batch" and (current_w != global_target_w or current_h != global_target_h):
                    raise ValueError(
                        f"\n\n\u3010\u5c3a\u5bf8\u51b2\u7a81\u62a5\u9519\u3011\n"
                        f"\u56fe\u7247 '{name}' ({current_w}x{current_h}) \u4e0e\u7b2c\u4e00\u5f20\u56fe "
                        f"({global_target_w}x{global_target_h}) \u5c3a\u5bf8\u4e0d\u4e00\u81f4\uff01\n\n"
                        f"\u89e3\u51b3\u65b9\u6848\uff1a\u5982\u679c\u56fe\u7247\u5c3a\u5bf8\u4e0d\u540c\uff0c"
                        f"\u8bf7\u4f7f\u7528\u9010\u5f20\u6267\u884c\u6216\u6267\u884c\u9996\u5f20\uff0c"
                        f"\u4e0d\u8981\u4f7f\u7528 batch \u4e00\u6b21\u62fc\u63a5\u3002\n"
                    )
    ...
    @classmethod
    def IS_CHANGED(s, image_list: str, max_images: int, mode: str, index: int = None, append=None, control=None):
        m = hashlib.sha256()
        control_data = _parse_control(control)
        control_action = _control_action(control_data, control)
        if _is_single_action(control_action):
    ...
    @classmethod
    def VALIDATE_INPUTS(s, image_list: str, max_images: int, mode: str, index: int = None, append=None, control=None):
        control_data = _parse_control(control)
        control_action = _control_action(control_data, control)
        if _is_single_action(control_action):
            mode = "single"
